...
export function detectCandlestickPatterns(candles: Candle[]): CandlestickPattern[] {
  ...
  for (let i = 1; i < candles.length; i++) {
    ...
    // Bullish Engulfing: current body fully engulfs previous body, bull after bear
    if (i >= 1) {
      ...
      // Bearish Engulfing: current body fully engulfs previous body, bear after bull
      if (!isBullish(curr) && isBullish(prev) && body > prevBody &&
          curr.open >= prev.close && curr.close <= prev.open) {
...
// Get only the most recent patterns (last N candles)
